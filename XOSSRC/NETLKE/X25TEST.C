...
long  block;
uchar seed;
char  prgname[] = "X25TEST";
char  inbuffer[BUFRSIZE];
...

{
    ...
    for (;;)
    {
        ...
        if ((rtn = svcIoInBlock(x25dev, inbuffer, BUFRSIZE)) < 0)
            femsg2(prgname, "Error inputting from X.25 device", rtn, NULL);
        ...
        do
        {
            if (*pnt != val)
            {
                printf("\n! Data error: a = %02.2X, x = %02.2X, p = %d, b = %d\n",
                        *pnt, val, MSGSIZE - cnt, block);
                exit(1);
            }
            pnt++;
            val++;
